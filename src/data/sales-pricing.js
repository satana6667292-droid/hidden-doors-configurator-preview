...
function configuredPriceBreakdown(){
  ...
  if(productKey==='double42'){
    ...
    lines.push(...configuredPriceUnknownSelectedExtras());
    sourceNote=price42SourceNote(price42Book(requestedType))+' · Створки рассчитаны по правилам полотна 42 мм. Общий каталог ПВХ: +2 300 ₽ по Опт 2 отдельно на каждую створку, где он выбран (две створки = +4 600 ₽ по Опт 2). Двустворчатый короб: две стоевые части сохраняют высотную логику короба 42; верх = (W₁ + W₂ + 10 мм) × ставка профиля, Опт 2: серый 1 076 ₽/м, чёрный 1 253 ₽/м.';
  }
