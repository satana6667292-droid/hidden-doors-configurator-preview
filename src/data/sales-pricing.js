...
const SALES_PRICE_COVERAGE=Object.freeze([
  ...
  Object.freeze({block:'Погонаж 42',area:'Профиль короба 42 + торцы BC3/C4',status:'connected',note:'Короб: Opt 2 серый 1 076 ₽/м, чёрный 1 253 ₽/м. BC3/C4: единая временная база закупа 1 638 ₽/6 м +60% = Opt 2; далее Opt 1 +12,5%, Розница +50%. Под конкретную дверь — по фактическому метражу 2×(H+20)+2×(W+20)'}),
  ...
  return '<div class="pricing-admin-grid">'+
    ...
    '<div class="pricing-category-grid">'+
      [
        ...
        ['trim','Погонаж','36 / 42 / 59 подключены; короб и торец 59 считаются по метражу, хлысту и под конкретную дверь','ok'],
...
function renderPricingSimpleSection(id){
  const map={
    ...
    trim:['Погонаж','36 мм, 42 мм и 59 мм подключены. Короб 59 и торец 59 с четвертью имеют метраж, целый хлыст и расчёт под конкретную дверь.','Погонаж подключён'],
...
function configuredPriceCompanionLabel(item){
  ...
  if(key==='DOOR-STOPPER'){
    const leaf=String(item?.leaf||'').trim();
    return (leaf?'Стопор · '+leaf:'Стопор')+(name?' · '+name:'');
  }
  if(key==='DOOR-THRESHOLD'){
    const leaf=String(item?.leaf||'').trim();
    return (leaf?'Автопорог · '+leaf:'Автопорог')+(name?' · '+name:'');
  }
  if(key==='DOOR-CLOSER')return name?'Доводчик · '+name:'Доводчик';
  if(key==='DOOR-VENT-GRILLE'){
    const leaf=String(item?.leaf||'').trim();
    return (leaf?'Вентиляционная решётка · '+leaf:'Вентиляционная решётка')+(name?' · '+name:'');
  }
  if(key==='DOOR-ADDITIONAL-ELEMENT'){
    const leaf=String(item?.leaf||'').trim();
    return (leaf?'Дополнительный элемент · '+leaf:'Дополнительный элемент')+(name?' · '+name:'');
  }
...
function configuredPriceLineFromCompanion(item,requestedType=activeSalesPriceType()){
  ...
  if(actualType&&normalizeSalesPriceType(actualType)!==normalizeSalesPriceType(requestedType)){
    noteParts.push('Для этой позиции отдельная оптовая цена пока не утверждена; в расчёте используется '+salesPriceTypeLabel(actualType)+'.');
  }
...
function configuredPrice42LeafLine(prefix,width,label,priceType=activeSalesPriceType()){
  ...
  const edgeKey=price42EdgeKey(product()==='double42'?($('DoubleEdgeColor')?.value||''):($(prefix+'EdgeColor')?.value||''));
  if(!frameKey||!edgeKey){
...
function configuredPriceUnknownSelectedExtras(existingLabels=new Set()){
  ...
  const skip=new Set(['Створка с замком','Активная створка','Пассивная створка','Ответная часть замка']);
  const represented=new Set([
    'Петли','Замок','Ручка','Ручка · левая створка','Ручка · правая створка','Завертка','Завёртка','Цилиндр','Стопор','Автопорог','Доводчик',
    'Запил короба под 45°','Ригель','Вентрешётка','Вентрешётка · левая створка','Вентрешётка · правая створка'
  ]);
  for(const label of existingLabels)represented.add(label);
  if(typeof companionItems==='function'){
    const companions=companionItems();
    for(const item of companions){
      if(String(item?.baseKey||item?.key||'')!=='DOOR-ADDITIONAL-ELEMENT')continue;
      const leaf=String(item?.leaf||'');
      if(leaf==='Левая створка')represented.add('Доп. элемент · левая створка');
      else if(leaf==='Правая створка')represented.add('Доп. элемент · правая створка');
      else represented.add('Доп. элемент');
    }
  }
...
function configuredPriceBreakdown(){
  ...
  if(productKey==='trim42'){
    const calc=typeof price42TrimProfileCalculation==='function'
      ?price42TrimProfileCalculation(price42TrimProfileCurrentState(),requestedType)
      :null;
    const item=typeof trim42Item==='function'?trim42Item():'';
    lines.push(configuredPriceLine({
      key:'trim42-profile',
      label:calc?.ok?calc.label:(item||'Погонаж 42'),
      qty:1,
      unit:calc?.ok?calc.unit:'шт.',
      unitPrice:calc?.ok?calc.price:null,
      priceKnown:!!calc?.ok&&Number.isFinite(Number(calc.price)),
      actualPriceType:calc?.ok?requestedType:null,
      note:calc?.ok
        ?(calc.formula+(calc.color?' · цвет '+calc.color:''))
        :(calc?.reason||'Цена этой позиции погонажа 42 пока не согласована.')
    }));
    sourceNote=calc?.ok
      ?((item==='Торец BC3'||item==='Торец C4')
        ?'Погонаж 42 · BC3/C4. Общая временная ставка: закуп 1 638 ₽ за хлыст 6 м по счёту АЛКОР №1356 от 23.09.2026; Opt 2 = закуп +60%; Opt 1 = Opt 2 +12,5%; Розница = Opt 2 +50%.'
        :'Погонаж 42 · профиль короба. Opt 2 считается по фактической длине из действующей ставки профиля: серый '+PRICE42_BOX_LINEAR_OPT2.gray+' ₽/м, чёрный '+PRICE42_BOX_LINEAR_OPT2.black+' ₽/м. Opt 1 = Opt 2 +12,5%; Розница = Opt 2 +50%.')
      :'Погонаж 42 · цена выбранной позиции не рассчитана.';
  }

  if(productKey==='trim59'){
    const calc=typeof price59TrimProfileCalculation==='function'
      ?price59TrimProfileCalculation(price59TrimProfileCurrentState(),requestedType)
      :null;
    const item=typeof trim59Item==='function'?trim59Item():'';
    lines.push(configuredPriceLine({
      key:'trim59-profile',
      label:calc?.ok?calc.label:(item||'Погонаж 59'),
      qty:1,
      unit:calc?.ok?calc.unit:'шт.',
      unitPrice:calc?.ok?calc.price:null,
      priceKnown:!!calc?.ok&&Number.isFinite(Number(calc.price)),
      actualPriceType:calc?.ok?requestedType:null,
      note:calc?.ok
        ?calc.formula+' · цвет '+calc.color
        :(calc?.reason||'Цена этой позиции погонажа 59 пока не согласована.')
    }));
    sourceNote=calc?.ok
      ?(item==='Торец 59 с четвертью'
  ...
  if(productKey==='installation'){
    ...
    const price=typeof hardwareSalesPrice==='function'
      ?hardwareSalesPrice('Монтаж и комплектующие',item,'retail')
      :null;
    lines.push(configuredPriceLine({
      key:'installation-item',
      label:item||'Монтаж и комплектующие',
      qty:1,unit:'шт.',
      unitPrice:price,
      priceKnown:price!==null&&price!==undefined&&Number.isFinite(Number(price)),
      actualPriceType:price!==null?'retail':null,
      note:price!==null
        ?'Утверждённая розничная цена. До отдельного расчёта Opt 1 / Opt 2 в оптовом заказе используется эта же розница.'
        :'Продажная цена для выбранной позиции пока не зафиксирована.'
    }));
    sourceNote='Монтаж и комплектующие: комплект монтажника 42 мм — 75 ₽; комплект монтажника 59 мм — 75 ₽; гибкий переход ABLOY EA281 хром — 800 ₽. Монтажные сухари скрыты.';
  }

  if(productKey==='additionalElement'){
    const type=$('additionalType')?.value||'';
    const item=$('catalogItem')?.value||'';
    const isVent=type==='Вентиляционные решётки';
    const isPorthole=type==='Иллюминаторы';
    let price=null;
    if(typeof hardwareSalesPrice==='function'){
      if(isVent)price=hardwareSalesPrice('Вентиляционные решетки',item,'retail');
      else price=hardwareSalesPrice('Доп.фурнитура',item,'retail');
    }
    lines.push(configuredPriceLine({
      key:'additional-element',
      label:item||type||'Дополнительный элемент',
      qty:1,unit:'шт.',
      unitPrice:price,
      priceKnown:price!==null&&price!==undefined&&Number.isFinite(Number(price)),
      actualPriceType:price!==null?'retail':null,
      note:price!==null
        ?'Если для позиции не утверждены Opt 1 / Opt 2, она используется в любом типе заказа по зафиксированной розничной цене.'
        :'Продажная цена для выбранного дополнительного элемента пока не зафиксирована.'
    }));
    sourceNote=isVent
      ?'Вентиляционные решётки · розничные цены зафиксированы по BASE каталога Bitrix24: Profiledoors 4 300 ₽; GTV 245×60 белая / серебристая / чёрная — по 610 ₽; металлическая 600×40 белая — 1 220 ₽.'
      :isPorthole
        ?'Иллюминаторы · розничные цены зафиксированы по BASE Bitrix24: 250 мм матовая латунь — 15 800 ₽; декоративный 350 мм — 16 160 ₽; нержавеющая сталь 350 мм матовый хром — 9 000 ₽. До утверждения оптовых цен во всех типах заказа используется розница.'
        :'Дополнительные элементы двери · позиции с зафиксированной BASE-ценой используются как розничные; Opt 1 / Opt 2 подключаются только после отдельного утверждения.';
  }
  ...
  if(productKey==='double42'){
    ...
    lines.push(...configuredPriceUnknownSelectedExtras());
    sourceNote=price42SourceNote(price42Book(requestedType))+' · Обе створки всегда используют один общий тип каркаса: либо обе на фанерном каркасе, либо обе на алюминиевом; смешанное исполнение не допускается. Роль активной/пассивной створки не связана с шириной: активной может быть любая из двух створок независимо от их размеров. Ограничения ширины применяются только к каждой створке отдельно; отдельного максимума или минимума по суммарной ширине двух створок нет. Признак всего комплекта «Стандарт» используется только при H=2000 мм и стандартной ширине 600/700/800/900 мм у обеих створок; если хотя бы одна створка или высота нестандартная, весь комплект получает «Нестандарт». Створки рассчитаны по правилам полотна 42 мм. Общий каталог ПВХ: +2 300 ₽ по Опт 2 отдельно на каждую створку, где он выбран (две створки = +4 600 ₽ по Опт 2). Эмаль: каждая створка отдельно, H×W×4 000 ₽/м²×число окрашиваемых сторон; ставка одинакова для Опт 2 / Опт 1 / Розницы. Порошковая покраска: весь выбранный под RAL алюминиевый профиль двустворчатой 42 (торцы обеих створок и короб, если он порошковый) суммируется по фактическому метражу, затем +10% технологического запаса × 460 ₽/м.п.; ставка одинакова для всех типов цен. Запил короба под 45°: 1 100 ₽ за весь комплект, фиксировано для всех типов цен. Замок: один на активной створке, модель и цена наследуют обычную 42; на противоположной створке только ответная часть, второй замок автоматически не добавляется. Ригель пассивной створки: PUNTO DHM-01 SN, 1 шт.; Опт 2 = 92 ₽, Опт 1 = 104 ₽, Розница = 138 ₽. Петли: для левой и правой створки выбираются независимо — модели и цвета могут отличаться; количество рассчитывается отдельно для каждой створки по её собственным параметрам и нагрузке, а не единым числом на весь комплект. Текущая утверждённая логика 42 мм без стекла/зеркала даёт рекомендованную K8060 и 2 петли на створку в действующем диапазоне; менеджер может изменить модель/цвет и количество отдельно. Если для конкретной створки в будущем будет разрешено тяжёлое исполнение с отдельной утверждённой нормой, повышенное количество применяется только к этой створке. Ручки: левая и правая створки имеют независимый выбор модели. При первом выборе ручки на одной створке та же модель автоматически подставляется на вторую, если там ещё стоит «Без ручки». После автоподстановки менеджер может независимо заменить ручку второй створки на другую модель или выбрать «Без ручки»; уже вручную выбранную другую ручку автокопирование не перезаписывает. Каждая выбранная ручка считается отдельной строкой по своей створке. Завёртка / WC-фиксатор к двустворчатой двери не привязывается и в её расчёт не входит; при необходимости добавляется в заказ отдельной товарной позицией из каталога фурнитуры. Если выбран замок типа «Под цилиндр», в карточке двустворчатой 42 обязателен 1 совместимый цилиндр; показываются только цилиндры для обычного конструктива 42 мм (60 мм, симметрия 30/30). Второй цилиндр автоматически не добавляется — при необходимости менеджер выбирает его отдельно во вкладке «Фурнитура». Стопоры выбираются отдельно для левой и правой створки: можно выбрать два разных, один или ни одного; каждый товар и фрезеровка под скрытый стопор считаются по своей створке. Автоматические пороги также выбираются отдельно на левую и правую створку: можно два, один или ни одного; каждый выбранный порог и фрезеровка 1 840 ₽ считаются отдельно по своей створке. Доводчик: один на весь комплект двустворчатой двери, без разделения по створкам. Вентиляционные решётки выбираются отдельно для левой и правой створки: можно две, одну или ни одной; модель на каждой створке может отличаться. Для каждой выбранной решётки автоматически включается отдельная платная обработка «Врезка вентиляционной решётки» = 1 500 ₽ на соответствующую створку; галочку обработки можно снять. Если выбраны две решётки, начисляются две обработки по 1 500 ₽. Цена самой вентиляционной решётки берётся из зафиксированной розничной BASE-цены Bitrix24; Opt 1 / Opt 2 для этой группы пока не утверждены. Иллюминатор / декоративный элемент также выбирается отдельно по каждой створке. Для распознанного иллюминатора врезка считается отдельно по каждой створке по 1 955 ₽; для дверцы для животного — по 2 875 ₽ на каждую выбранную створку. Цена самой дополнительной номенклатуры без утверждённого источника не придумывается. Для двустворчатой 42 всегда используется схема «одна активная + одна пассивная створка»; режима «обе активные» нет. Активная створка выбирается независимо от наличия замка, противоположная автоматически становится пассивной. Активная/пассивная сторона является только характеристикой заказа и не входит в SKU, длинное название или ключ дублей номенклатуры. Для двустворчатой 42 открывание фиксировано конструктивом: левая створка — «левое на себя», правая — «правое на себя»; отдельного общего переключателя открывания нет. Активная створка выбирается независимо от этой фиксированной схемы. Цвет алюминиевого торца один на весь комплект обеих створок. Для СКУД-замка используется один замок и одна врезка СКУД только на активной створке; на пассивной — только ответная часть. Вторая врезка автоматически не добавляется. Цвет короба по умолчанию синхронизируется с общим цветом алюминиевого торца, но менеджер может вручную выбрать другой цвет; расчёт короба и его порошковой покраски после этого идёт по фактически выбранному цвету короба. При следующем изменении общего цвета торца короб снова получает его как новое значение по умолчанию. Если и общий торец, и короб выбраны в полимерно-порошковой покраске, их RAL всегда один и тот же: RAL короба автоматически берётся из общего RAL торца и отдельно не редактируется. Изменение только RAL торца не сбрасывает вручную выбранный тип/цвет короба. Двустворчатый короб: две стоевые части сохраняют высотную логику короба 42; верх = (W₁ + W₂ + 10 мм) × ставка профиля, Опт 2: серый 1 076 ₽/м, чёрный 1 253 ₽/м.';
  }
